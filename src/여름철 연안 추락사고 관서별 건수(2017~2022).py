...
dff = df[(dfm >= 6) & (dfm <= 9)]
df_agg = dff.groupby('해경서')['해경서'].size()
df_new = pd.DataFrame({"해경서": df_agg.index, "사고건수": df_agg.values})
...
plt.bar(df_new['해경서'],df_new['사고건수'],color=colors)
plt.title("여름철 연안 추락사고 관서별 건수(2017~2022)")
plt.xlabel("해경서")
plt.xticks(rotation=45) 
plt.ylabel("건수")

for i in range(19):
        plt.text(df_new['해경서'][i], df_new['사고건수'][i], str(df_new['사고건수'][i]), ha='center', va='bottom')
plt.show()